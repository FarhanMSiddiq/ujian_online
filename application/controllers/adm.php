...
class Adm extends CI_Controller {
	...
	public function index() {
		...
		$a['jml_guru'] = $this->db->get('m_guru')->num_rows();
		$a['jml_siswa'] = $this->db->get('m_siswa')->num_rows();
		$a['jml_mapel'] = $this->db->get('m_mapel')->num_rows();
		$a['jml_soal'] = $this->db->get('m_soal')->num_rows();

		$cart_pie_data = array();
		$cart_pie_data2 = array();

		
		$jml_guru = array('y' => $a['jml_guru'] , 'name' => 'Guru' );
		$jml_siswa = array('y' => $a['jml_siswa'] , 'name' => 'Siswa' );
		$jml_mapel = array('y' => $a['jml_mapel'] , 'name' => 'Mapel' );
		$jml_soal = array('y' => $a['jml_soal'] , 'name' => 'Soal' );

		array_push($cart_pie_data,$jml_guru,$jml_siswa,$jml_mapel,$jml_soal);

		$a['chart1'] = json_encode($cart_pie_data);

		$mapel = $this->db->get('m_mapel')->result();

		foreach ($mapel as $key => $value) {
			$soal = $this->db->get_where('m_soal',array('id_mapel'=>$value->id))->num_rows();
			
			$array_soalms = array('y'=>$soal,'name'=>$value->nama);

			array_push($cart_pie_data2,$array_soalms);
		}

		$a['chart2'] = json_encode($cart_pie_data2);


		$a['jml_ujian'] = $this->db->get('tr_guru_tes')->num_rows();
		$a['blm_dikerjakan'] = $this->db->query("SELECT 
		a.id, a.nama_ujian, a.jumlah_soal, a.waktu,
		b.nama nmmapel,
		c.nama nmguru,d.nilai,
		...
		LEFT JOIN tr_ikut_ujian d ON CONCAT('".$a['sess_konid']."',a.id) = CONCAT(d.id_user,d.id_tes) WHERE NOT EXISTS (SELECT * FROM tr_ikut_ujian WHERE a.id = tr_ikut_ujian.id_tes AND tr_ikut_ujian.status = 'N' AND tr_ikut_ujian.id_user = '".$a['sess_konid']."') 
		ORDER BY a.id DESC")->num_rows();

		$a['sudah_dikerjakan'] = $this->db->query("SELECT a.*,b.nama nmmapel,c.nama nmguru,d.nama_ujian, d.jumlah_soal, d.waktu
		FROM tr_ikut_ujian a 
		INNER JOIN tr_guru_tes d ON d.id = a.id_tes
		INNER JOIN m_mapel b ON d.id_mapel = b.id
		INNER JOIN m_guru c ON d.id_guru = c.id
		WHERE  a.id_user = '".$a['sess_konid']."'
		ORDER BY a.id DESC")->num_rows();
		...
		if ($uri3 == "det") {
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[] = '<div class="btn-group">
                          <a href="#" onclick="return m_siswa_e('.$d['id'].');" class="btn btn-info btn-xs"><i class="mdi mdi-pencil mr-2"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_siswa_h('.$d['id'].');" class="btn btn-danger btn-xs"><i class="mdi mdi-delete mr-2"></i> &nbsp;&nbsp;Hapus</a>
                         ';

                if ($d['ada'] == "0") {
                  $data_ok[4] .= '<a href="#" onclick="return m_siswa_u('.$d['id'].');" class="btn btn-info btn-xs"><i class="mdi mdi-account mr-2"></i> &nbsp;&nbsp;Aktifkan User</a>';
                } else {
                  $data_ok[4] .= '<a href="#" onclick="return m_siswa_ur('.$d['id'].');" class="btn btn-warning btn-xs"><i class="mdi mdi-sort-variant-lock mr-2"></i> &nbsp;&nbsp;Reset Password</a>';
                }
			...
			if (!empty($q_get_user)) {
				foreach ($q_get_user as $j) {
					if ($j['usernya'] == "N") {
						$this->db->query("INSERT INTO m_admin VALUES (null, '".$j['nama']."', md5('".$j['nim']."'), 'siswa', '".$j['id']."')");
						$jml_aktif++;
					}
				}
			}
		...
		if ($uri3 == "det") {
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[3] = '<div class="btn-group">
                          <a href="#" onclick="return m_guru_e('.$d['id'].');" class="btn btn-info btn-xs"><i class="mdi mdi-pencil mr-2"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_guru_h('.$d['id'].');" class="btn btn-danger btn-xs"><i class="mdi mdi-delete mr-2"></i> &nbsp;&nbsp;Hapus</a>
                          <a href="#" onclick="return m_guru_matkul('.$d['id'].');" class="btn btn-success btn-xs"><i class="mdi mdi-format-list-bulleted mr-2"></i> &nbsp;&nbsp;MaPel Diampu</a>
                         ';

                if ($d['ada'] == "0") {
                  $data_ok[3] .= '<a href="#" onclick="return m_guru_u('.$d['id'].');" class="btn btn-info btn-xs"><i class="mdi mdi-account mr-2"></i> &nbsp;&nbsp;Aktif User</a>';
                } else {
                  $data_ok[3] .= '<a href="#" onclick="return m_guru_ur('.$d['id'].');" class="btn btn-warning btn-xs"><i class="mdi mdi-sort-variant-lock mr-2"></i> &nbsp;&nbsp;Reset Pass</a>';
                }
	...
	public function m_mapel() {
		...
		if ($uri3 == "det") {
			...
			$ket 	= "";
			if ($p->id != 0) {
				$this->db->query("UPDATE m_mapel SET nama = '".bersih($p,"nama")."', detail = '".bersih($p,"detail")."',thumbnail = '".bersih($p,"thumbnail")."'
								WHERE id = '".bersih($p,"id")."'");
				$ket = "edit";
			} else {
				$ket = "tambah";
				$this->db->query("INSERT INTO m_mapel VALUES (null, '".bersih($p,"nama")."','".bersih($p,"detail")."','".bersih($p,"thumbnail")."')");
			}
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[2] = '<div class="btn-group">
                          <a href="#" onclick="return m_mapel_e('.$d['id'].');" class="btn btn-info btn-xs"><i class="mdi mdi-pencil mr-2"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_mapel_h('.$d['id'].');" class="btn btn-danger btn-xs"><i class="mdi mdi-delete mr-2"></i> &nbsp;&nbsp;Hapus</a>
                         ';
		...
		if ($uri3 == "det") {
			...
			if (!empty($data)) {
		        ...
		        foreach ($q_datanya as $d) {
		        	...
		        	$persen_salah = $total > 0 ? (($jml_salah / $total) * 100) : 0; 

				$data_ok = array();
				$data_ok[0] = $no++;
				$data_ok[1] = substr($d['soal'], 0, 70);
				$data_ok[2] = $d['nmmapel'].'<br>'.$d['nmguru'];
				$data_ok[3] = "Jml dipakai : ".($total)."<br>Benar: ".$jml_benar.", Salah: ".$jml_salah."<br>Persentase benar : ".number_format($persen_benar)." % ,"."<br>Persentase salah : ".number_format($persen_salah)." %";
				$data_ok[4] = '<div class="btn-group">
				  <a href="'.base_url().'adm/m_soal/edit/'.$d['id'].'" class="btn btn-info btn-xs"><i class="mdi mdi-pencil mr-2"></i> &nbsp;&nbsp;Edit</a>
				  <a href="#" onclick="return m_soal_h('.$d['id'].');" class="btn btn-danger btn-xs"><i class="mdi mdi-delete mr-2"></i> &nbsp;&nbsp;Hapus</a>
				 ';
	...
	public function m_pelajaran() {
		...
		if ($uri3 == "det") {
			$a = $this->db->query("SELECT * FROM m_pelajaran WHERE id = '$uri4' ORDER BY id DESC")->row();
			j($a);
			exit();
		} else if ($uri3 == "import") {
			$a['p']	= "f_pelajaran_import";
		} else if ($uri3 == "hapus_gambar") {
			$nama_gambar = $this->db->query("SELECT file FROM m_pelajaran WHERE id = '".$uri5."'")->row();
			$this->db->query("UPDATE m_pelajaran SET file = '', tipe_file = '' WHERE id = '".$uri5."'");
			@unlink("./upload/gambar_soal/".$nama_gambar->file);
			redirect('adm/m_pelajaran/pilih_mapel/'.$uri4);
		} else if ($uri3 == "pilih_mapel") {
			if ($a['sess_level'] == "guru") {
				$a['data'] = $this->db->query("SELECT m_pelajaran.*, m_guru.nama AS nama_guru FROM m_pelajaran INNER JOIN m_guru ON m_pelajaran.id_guru = m_guru.id WHERE m_pelajaran.id_guru = '".$a['sess_konid']."' AND m_pelajaran.id_mapel = '$uri4' ORDER BY id DESC")->result();
			} else {
				$a['data'] = $this->db->query("SELECT m_pelajaran.*, m_guru.nama AS nama_guru FROM m_pelajaran INNER JOIN m_guru ON m_pelajaran.id_guru = m_guru.id WHERE m_pelajaran.id_mapel = '$uri4' ORDER BY id DESC")->result();
			}
			//echo $this->db->last_query();
			$a['p']	= "m_pelajaran";
		} else if ($uri3 == "simpan") {
			...
			if ($__mode == "edit") {
				$this->db->where("id", $p['id']);
				$this->db->update("m_pelajaran", $pdata);
				$__id_soal = $p['id'];
			} else {
				$this->db->insert("m_pelajaran", $pdata);
				$get_id_akhir = $this->db->query("SELECT MAX(id) maks FROM m_pelajaran LIMIT 1")->row_array();
				$__id_soal = $get_id_akhir['maks'];
			}
			...
			//ambil data awal
			$get_opsi_awal = $this->db->query("SELECT opsi_a, opsi_b, opsi_c, opsi_d, opsi_e FROM m_pelajaran WHERE id = '".$__id_soal."'")->row_array();
			...
			$this->db->where("id", $__id_soal);
			$this->db->update("m_pelajaran", $data_simpan);
			...
			redirect('adm/m_pelajaran/pilih_mapel/'.$p['id_mapel']);
		} else if ($uri3 == "edit") {
			$a['opsij'] = array(""=>"Jawaban","A"=>"A","B"=>"B","C"=>"C","D"=>"D","E"=>"E");
			...
			if ($uri4 == 0) {
				...
				$a['d'] = $this->db->query("SELECT m_pelajaran.*, 'edit' AS mode FROM m_pelajaran WHERE id = '$uri4'")->row_array();
			...
			$nama_gambar = $this->db->query("SELECT id_mapel, file, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e FROM m_pelajaran WHERE id = '".$uri4."'")->row();
			$pc_opsi_a = explode("#####", $nama_gambar->opsi_a);
			$pc_opsi_b = explode("#####", $nama_gambar->opsi_b);
			$pc_opsi_c = explode("#####", $nama_gambar->opsi_c);
			$pc_opsi_d = explode("#####", $nama_gambar->opsi_d);
			$pc_opsi_e = explode("#####", $nama_gambar->opsi_e);
			$this->db->query("DELETE FROM m_pelajaran WHERE id = '".$uri4."'");
			@unlink("./upload/gambar_soal/".$nama_gambar->file);
			@unlink("./upload/gambar_soal/".$pc_opsi_a[0]);
			@unlink("./upload/gambar_soal/".$pc_opsi_b[0]);
			@unlink("./upload/gambar_soal/".$pc_opsi_c[0]);
			...
			exit();
				...
				$data_ok[3] = "Jml dipakai : ".($total)."<br>Benar: ".$jml_benar.", Salah: ".$jml_salah."<br>Persentase benar : ".number_format($persen_benar)." % ,"."<br>Persentase salah : ".number_format($persen_salah)." %";
				$data_ok[4] = '<div class="btn-group">
				  <a href="'.base_url().'adm/m_pelajaran/edit/'.$d['id'].'" class="btn btn-info btn-xs"><i class="mdi mdi-pencil mr-2"></i> &nbsp;&nbsp;Edit</a>
				  <a href="#" onclick="return m_pelajaran_h('.$d['id'].');" class="btn btn-danger btn-xs"><i class="mdi mdi-delete mr-2"></i> &nbsp;&nbsp;Hapus</a>
				 ';
	...
	public function m_ujian() {
		...
		if ($a['sess_level'] == "guru") {
			...
			$a['p_mapel'] = obj_to_array($this->db->query("SELECT * FROM m_mapel WHERE id IN (SELECT id_mapel FROM tr_guru_mapel)")->result(), "id,nama");
		}
		...
		if ($uri3 == "det") {
			...
			j($are);
			exit();
		} else if ($uri3 == "atur_soal") {
			$a['id_ujian'] = $uri4;
			$a['p'] = "atur_soal";
		}else if ($uri3 == "data_atur_soal") {
				$id = $uri4;
				$start = $this->input->post('start');
		        $length = $this->input->post('length');
		        $draw = $this->input->post('draw');
		        ...
		        $d_total_row = $this->db->query("SELECT tr_package_ujian.id as id_package,
												tr_package_ujian.id_ujian,tr_package_ujian.id_soal ,m_soal.id,m_soal.soal 
												FROM tr_package_ujian,m_soal WHERE tr_package_ujian.id_soal = m_soal.id AND 
												tr_package_ujian.id_ujian = '$id' AND m_soal.soal LIKE '%".$search['value']."%'")->num_rows();
		    	
		    	//echo $this->db->last_query();

		        $q_datanya = $this->db->query("SELECT tr_package_ujian.id as id_package,
											tr_package_ujian.id_ujian,tr_package_ujian.id_soal ,m_soal.id,m_soal.soal 
											FROM tr_package_ujian,m_soal WHERE tr_package_ujian.id_soal = m_soal.id AND 
											tr_package_ujian.id_ujian = '$id' AND m_soal.soal LIKE '%".$search['value']."%'
											ORDER BY tr_package_ujian.id DESC LIMIT ".$start.", ".$length."")->result_array();
		        $data = array();
				$no = ($start+1);
		        ...
		        foreach ($q_datanya as $d) {	
		            ...
		            $data_ok[2] = '
		            	<div class="btn-group">
                          <a href="'.base_url('adm/m_ujian/hapus_soal_ujian?1=').''.$d['id'].'&2='.$uri4.'" class="btn btn-danger btn-xs"><i class="mdi mdi-delete mr-2"></i> &nbsp;&nbsp;Hapus</a>
                        </div>
	                         ';
		        ...
		        j($json_data);
		        exit;
		}else if ($uri3 == "tambah_soal_ujian") {
			$id_soal = $_GET['1'];
			$id_ujian = $_GET['2'];

			$cek = $this->db->query("SELECT * FROM tr_guru_tes WHERE id='$id_ujian'")->row();

			$package = $this->db->query("SELECT * FROM tr_package_ujian WHERE id_ujian='$id_ujian'")->num_rows();

			if($cek->jumlah_soal == $package){
				?><script>alert('jumlah soal mu sudah max'); window.location.href='<?php echo base_url('adm/m_ujian/atur_soal'); ?>/<?php echo $id_ujian; ?>';</script><?php
			}else{
				$tambah = $this->db->query("INSERT INTO tr_package_ujian VALUES ('','$id_soal','$id_ujian')");

				if($tambah){
					?><script>window.location.href='<?php echo base_url('adm/m_ujian/atur_soal'); ?>/<?php echo $id_ujian; ?>';</script><?php
				}
			}

			

		}else if ($uri3 == "hapus_soal_ujian") {
			$id = $_GET['1'];
			$id_ujian = $_GET['2'];

			$hapus = $this->db->query("DELETE FROM tr_package_ujian WHERE id ='$id'");

			if($hapus){
				?><script>window.location.href='<?php echo base_url('adm/m_ujian/atur_soal'); ?>/<?php echo $id_ujian; ?>';</script><?php
			}

		}else if ($uri3 == "bank_soal") {
			...
			$d_total_row = $this->db->query("SELECT a.*
											...
											INNER JOIN m_guru b ON a.id_guru = b.id
											INNER JOIN m_mapel c ON a.id_mapel = c.id
											WHERE  NOT EXISTS (SELECT * FROM tr_package_ujian WHERE a.id = tr_package_ujian.id_soal AND tr_package_ujian.id_ujian = ".$uri4." ) AND ".$wh." (a.soal LIKE '%".$search['value']."%' 
											OR b.nama LIKE '%".$search['value']."%' 
											OR c.nama LIKE '%".$search['value']."%')")->num_rows();
			...
			foreach ($q_datanya as $d) {	
				...
				$data_ok[2] = '
					<div class="btn-group">
					  <a href="'.base_url('adm/m_ujian/tambah_soal_ujian?1=').''.$d['id'].'&2='.$uri4.'" class="btn btn-success btn-xs"><i class="mdi mdi-plus mr-2"></i> &nbsp;&nbsp;Tambah</a>
					</div>
						 ';
				...
				if ($a['sess_level'] == "guru") {
					if ($p->id != 0) {
						...
						$this->db->query("INSERT INTO tr_guru_tes VALUES (
							...
							'$token',
							'https://image.freepik.com/free-vector/quiz-background-with-items-flat-design_23-2147599082.jpg')");
					}
				} else if ($a['sess_level'] == "admin") {
					if ($p->id != 0) {
						$this->db->query("UPDATE tr_guru_tes SET 
			...
			exit();
			   ...
			   $data = array();
		        ...
		        foreach ($q_datanya as $d) {
		            ...
		            $data_ok[6] = '
		            	<div class="btn-group">
                          <a href="#" onclick="return m_ujian_e('.$d['id'].');" class="btn btn-info btn-xs"><i class="mdi mdi-pencil mr-2"></i> &nbsp;&nbsp;Edit</a>
						  <a href="#" onclick="return m_ujian_h('.$d['id'].');" class="btn btn-danger btn-xs"><i class="mdi mdi-delete mr-2"></i> &nbsp;&nbsp;Hapus</a>
						  <a href="'.base_url('adm/m_ujian/atur_soal').'/'.$d['id'].'" class="btn btn-success btn-xs"><i class="mdi mdi-pencil mr-2"></i> &nbsp;&nbsp;&nbsp;Atur Soal</a>
                        </div>
	                         ';
	...
	public function h_ujian() {
		$this->cek_aktif();
		cek_hakakses(array("guru","admin","pengawas"), $this->session->userdata('admin_level'));
		...
		$wh_1 = $a['sess_level'] == "admin" || $a['sess_level'] == "pengawas"  ? "" : " AND a.id_guru = '".$a['sess_konid']."'";
		...
		if ($uri3 == "det") {
	        ...
	        $q_datanya = $this->db->query("
	        	SELECT a.id, b.nama,b.nim, a.nilai, a.jml_benar, a.nilai_bobot,(c.jumlah_soal-a.jml_benar) as jml_salah
				FROM tr_ikut_ujian a
				INNER JOIN m_siswa b ON a.id_user = b.id
				INNER JOIN tr_guru_tes c ON a.id_tes = c.id
				WHERE a.id_tes = '$uri4' 
				AND b.nama LIKE '%".$search['value']."%' ORDER BY b.jurusan , b.nama ASC LIMIT ".$start.", ".$length."")->result_array();
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[5] = $d['nilai'];
	            // $data_ok[6] = '<a href="'.base_url().'adm/h_ujian/batalkan_ujian/'.$d['id'].'/'.$this->uri->segment(4).'" class="btn btn-danger btn-xs" onclick="return confirm(\'Anda yakin...?\');"><i class="mdi mdi-delete mr-2"></i> &nbsp;&nbsp;Batalkan Ujian</a>';
				$data_ok[6] = '<div class="checkbox checkbox-primary checkbox-single">
								<input type="checkbox" id="singleCheckbox'.$d['id'].'" name="check[]" value="'.$d['id'].'"  aria-label="Single checkbox Two">
								<label></label>
							</div>';
	            $data[] = $data_ok;
	        }
			...
			foreach ($check as $key => $value) {
				$this->db->query("DELETE FROM tr_ikut_ujian WHERE id = '$value'");
			}
	
			// $this->db->query("DELETE FROM tr_ikut_ujian WHERE id = '$uri4'");
			redirect('adm/h_ujian/det/'.$_POST['id']);
		} else if ($uri3 == "data") {
			$start = $this->input->post('start');
	        $length = $this->input->post('length');
	        $draw = $this->input->post('draw');
	...
	public function sudah_dikerjakan() {
		...
		$jeson = array();
		//$a['sess_konid']
		$a['data'] = $this->db->query("SELECT a.*,b.nama nmmapel,c.nama nmguru,d.nama_ujian, d.jumlah_soal, d.waktu
									FROM tr_ikut_ujian a 
									INNER JOIN tr_guru_tes d ON d.id = a.id_tes
									INNER JOIN m_mapel b ON d.id_mapel = b.id
									INNER JOIN m_guru c ON d.id_guru = c.id
	...
	public function ikut_ujian() {
		...
		if ($uri3 == "simpan_satu") {
			...
			$nilai = ($jumlah_benar / $jumlah_soal)  * 10;
			$nilai_bobot = ($nilai_bobot / $total_bobot)  * 10;
			...
			if ($cek_sdh_selesai < 1) {
				...
				if ($cek_sdh_ujian < 1)	{		
					$soal_urut_ok = array();
					$q_soal			= $this->db->query("SELECT tr_package_ujian.id as id_package,tr_package_ujian.id_ujian,tr_package_ujian.id_soal ,m_soal.id,m_soal.file ,m_soal.tipe_file,m_soal.soal,m_soal.opsi_a,m_soal.opsi_b,m_soal.opsi_c,m_soal.opsi_d,m_soal.opsi_e, '' as jawaban FROM tr_package_ujian,m_soal WHERE tr_package_ujian.id_soal = m_soal.id AND tr_package_ujian.id_ujian = '$uri4' ".$acakan."")->result();
					$i = 0;
					foreach ($q_soal as $s) {
				...
				if (!empty($soal_urut_ok)) {
				    foreach ($soal_urut_ok as $d) { 
                        ...
                        if($d->tipe_file == 'image/jpeg'){
                        $html .= $d->soal.'<div class="funkyradio">';    
                        }else{
				        $html .= $tampil_media.'<br>'.$d->soal.'<div class="funkyradio">';
                        }
	...
	public function act_login() {
		...
		if ($j_data === 1) {
			...
			} elseif($a_data->level == "pengawas") {
				$sess_nama_user = "Pengawas";
			}else{
